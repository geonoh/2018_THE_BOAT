...
void ServerFramework::InitServer() {
	...
	// 무브업뎃
	AddTimer(NULL, s_ev_move, GetTickCount() + TIME_UPDATE);
	// 1/20초마다 Position Send해주기
	AddTimer(NULL, s_ev_pos, GetTickCount() + TIME_POSITION);
}
...
void ServerFramework::AcceptPlayer() {
	...
	clients[client_id].overlapped_ex.type = s_ev_recv;
	...
	// 마우스 움직임을 받아야한다. 
	//AddTimer(client_id, s_ev_mouse, GetTickCount() + TIME_MOUSE);
...
void ServerFramework::ProcessPacket(int cl_id, char* packet) {
	...
	case CS_MOUSE_MOVE: {
		...
		AddTimer(cl_id, s_ev_mouse, GetTickCount() + 1, packet_buffer->look_vec);
		break;
	}
...
void ServerFramework::WorkerThread() {
	...
	while (true) {
		...
		if (overlapped_buffer->type == s_ev_recv) {
		...
		else if (overlapped_buffer->type == s_ev_pos) {
		...
		else if (overlapped_buffer->type == s_ev_collision) {
			// OBB 충돌체크  
		...
		else if (overlapped_buffer->type == s_ev_send) {
			delete overlapped_buffer;
		}
		else if (overlapped_buffer->type == s_ev_mouse) {
			//여기 PQCS로 처리해야함
			// 아 룩벡터 어카죠?;
			clients[client_id].look_vec = overlapped_buffer->look_vec;
		...
		else if (overlapped_buffer->type == s_ev_move) {
			for (int i = 0; i < MAXIMUM_PLAYER; ++i) {
				//client_lock.lock();
				//clients[i].client_lock.lock();

				if (clients[i].is_move_foward == true) {
					printf("마우스 벡터 x : %f, y : %f, z : %f \n",
						clients[i].look_vec.x, clients[i].look_vec.y, clients[i].look_vec.z);
					if (clients[i].is_running) {
						clients[i].z += PIXER_PER_METER * clients[i].look_vec.z * (RUN_SPEED * 0.1f);
						clients[i].x += PIXER_PER_METER * clients[i].look_vec.x * (RUN_SPEED * 0.1f);
					}
					else {
						clients[i].z += PIXER_PER_METER * clients[i].look_vec.z * (WALK_SPEED * 0.1f);
						clients[i].x += PIXER_PER_METER * clients[i].look_vec.x * (WALK_SPEED * 0.1f);
					}
				}
				if (clients[i].is_move_backward) {
					if (clients[i].is_running) {
						clients[i].z += (-1) * PIXER_PER_METER * clients[i].look_vec.z * (RUN_SPEED * 0.1f);
						clients[i].x += (-1) * PIXER_PER_METER * clients[i].look_vec.x * (RUN_SPEED * 0.1f);
					}
					else {
						clients[i].z += (-1) * PIXER_PER_METER * clients[i].look_vec.z * (WALK_SPEED * 0.1f);
						clients[i].x += (-1) * PIXER_PER_METER * clients[i].look_vec.x * (WALK_SPEED * 0.1f);
					}
				}
				if (clients[i].is_move_left) {
					if (clients[i].is_running) {
						clients[i].z += PIXER_PER_METER * clients[i].look_vec.x * (RUN_SPEED * 0.1f);
						clients[i].x += (-1) * PIXER_PER_METER * clients[i].look_vec.z * (RUN_SPEED * 0.1f);
					}
					else {
						clients[i].z += PIXER_PER_METER * clients[i].look_vec.x * (WALK_SPEED * 0.1f);
						clients[i].x += (-1) * PIXER_PER_METER * clients[i].look_vec.z * (WALK_SPEED * 0.1f);
					}
				}
				if (clients[i].is_move_right) {
					if (clients[i].is_running) {
						clients[i].z += (-1) * PIXER_PER_METER * clients[i].look_vec.x * (RUN_SPEED * 0.1f);
						clients[i].x += PIXER_PER_METER * clients[i].look_vec.z * (RUN_SPEED * 0.1f);
					}
					else {
						clients[i].z += (-1) * PIXER_PER_METER * clients[i].look_vec.x * (WALK_SPEED * 0.1f);
						clients[i].x += PIXER_PER_METER * clients[i].look_vec.z * (WALK_SPEED * 0.1f);
					}
		...
		else if (overlapped_buffer->type == s_ev_bullet_gen) {
			for (int i = 0; i < MAXIMUM_PLAYER; ++i) {
		...
		else if (overlapped_buffer->type == s_ev_bullet_move) {
			// i 가 플레이어
			// j 가 플레이어가 발사한 총알
			for (int i = 0; i < MAXIMUM_PLAYER; ++i) {
...
void ServerFramework::CallPQCS(ServerEvent& process_event, OverlappedExtensionSet* process_ovex) {
	PostQueuedCompletionStatus(iocp_handle, 1, process_event.id, &process_ovex->wsa_over);
}
...
void ServerFramework::TimerThread() {
	while (true) {
		...
		while (timer_queue.empty() == false) {
			...
			ServerEvent process_event = timer_queue.top();
			timer_queue.pop();
			OverlappedExtensionSet* process_ovex = new OverlappedExtensionSet;
			// 마우스 패킷경우에만 LookVector 전송해주기
			if (process_event.type == s_ev_mouse)
				process_ovex->look_vec = process_event.look_vec;
			process_ovex->type = process_event.type;
			CallPQCS(process_event, process_ovex);
		}
	}
...
void ServerFramework::AddTimer(int id, int type, unsigned int start_time) {
	timer_queue.push(ServerEvent{ id, type, start_time });
}

void ServerFramework::AddTimer(int id, int type, unsigned int start_time, XMFLOAT3& look_vec) {
	timer_queue.push(ServerEvent{ id, type, start_time, look_vec });
}