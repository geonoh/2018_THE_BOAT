...
void ServerMgr::ReadPacket() {
	...
	if (retval == 1) {
		...
		ErrorDisplay("[WSARecv] : ø°∑Ø ", err_code);
	}
...
void ServerMgr::ProcessPacket(char* ptr) {
	...
	case SC_ENTER_PLAYER: {
		...
		// √÷√  ºˆΩ≈ «ﬂ¥Ÿ¥¬ ¡§∫∏∏¶ πﬁ∞‘ µ«∏È my_client_idø° 
		// ºˆΩ≈«— client_id ¡§∫∏∏¶ ≥÷¥¬¥Ÿ.
		if (first_set_id)
			my_client_id = packets->id;
		
		cout << "[SC_ENTER_PLAYER] : " << packets->id << "«√∑π¿ÃæÓ ¿‘¿Â" << endl;
		break; }
	case SC_POS: {
		SC_PACKET_POS* packets = reinterpret_cast<SC_PACKET_POS*>(ptr);

		cout << "[SC_PACKET_POS] : " << packets->id << "«√∑π¿ÃæÓ ¿Ãµø" << endl;
		break; }
	}
}
...
>>>>>>> [ÏÑúÎ≤Ñ ÌîÑÎ†àÏûÑÏõåÌÅ¨] : ÌÇ§ push, down Ïóê Îî∞Î•∏ Ïù¥Îèô Î∞©Ïãù Íµ¨ÌòÑ [ÎçîÎØ∏ÌÅ¥ÎùºÏù¥Ïñ∏Ìä∏] : ÌÇ§ ÏûÖÎ†• Ï§ëÏ≤© ÏïàÎêòÎèÑÎ°ù ÏΩîÎìú ÏàòÏ†ï
		retval = WSASend(sock, &send_wsabuf, 1, &iobytes, 0, NULL, NULL);
		break;
	...
	case CS_MOUSE_MOVE:
		...
		// ø©±‚ø° √ﬂ∞°¿˚¿∏∑Œ player¿« look ∫§≈Õ∏¶ ∞∞¿Ã «ÿº≠ ∫∏≥ª¡‡æﬂ«—¥Ÿ. 
		retval = WSASend(sock, &send_wsabuf, 1, &iobytes, 0, NULL, NULL);
	}
	if (retval == 1) {
		int error_code = WSAGetLastError();
		ErrorDisplay("[WSASend] ø°∑Ø : ", error_code);
	}
