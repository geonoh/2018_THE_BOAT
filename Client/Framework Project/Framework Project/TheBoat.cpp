...
private:
	...
	float GetHillsHeight(int x, int z, void * pContext) const;
...
public:
	HeightMap * qwr;
	LPCTSTR FileName = _T("../Assets/Image/Terrain/HeightMap.raw");
};
...
//	mEyePos.y = mRadius * cosf(mPhi);
//
//	XMVECTOR pos = XMVectorSet(150, 150, mEyePos.z, 1.0f);
//	XMVECTOR target =/* XMVectorZero()*/XMVectorSet(70, 0, -70, 1.0f);
//	XMVECTOR up = XMVectorSet(0.0f, 1.0f, 0.0f, 0.0f);
//
//	XMMATRIX view = XMMatrixLookAtLH(pos, target, up);
...
//
//	for (int i = 0, z = 0; z < (0 + 257); z++)
//	{
//		for (int x = 0; x < (0 + 257); x++, i++)
//		{
...
void TheBoat::BuildLandGeometry()
{
	...
	for (int i = 0, z = 0; z < (0 + 257); z++)
	{
		for (int x = 0; x < (0 + 257); x++, i++)
		{
			...
			vertices[i].Color = OnGetColor(x, z, qwr);
			vertices[0].Pos.y = 0;
			vertices[256].Pos.y = 0;
			vertices[65792].Pos.y = 0;
			vertices[66048].Pos.y = 0;
