...
// ?テリアル?示
void GetMatrialData(FbxSurfaceMaterial* mat0)
{
	if (mat0 == nullptr)
	{
		return;
	...
	puts("");
	if (mat0->GetClassId().Is(FbxSurfaceLambert::ClassId))
	{
		printf("ランバ?ト?イプ\n");
	}
	else if (mat0->GetClassId().Is(FbxSurfacePhong::ClassId))
	{
		printf("フォン?イプ\n");
	}

	const FbxDouble3 lEmissive = GetMaterialProperty(mat0, FbxSurfaceMaterial::sEmissive, FbxSurfaceMaterial::sEmissiveFactor);
	printf("エ?ッシブカラ?:r = %f, g = %f, b = %f\n", lEmissive.mData[0], lEmissive.mData[1], lEmissive.mData[2]);

	const FbxDouble3 lAmbient = GetMaterialProperty(mat0, FbxSurfaceMaterial::sAmbient, FbxSurfaceMaterial::sAmbientFactor);
	printf("アンビエントカラ?:r = %f, g = %f, b = %f\n", lAmbient.mData[0], lAmbient.mData[1], lAmbient.mData[2]);

	const FbxDouble3 lDiffuse = GetMaterialProperty(mat0, FbxSurfaceMaterial::sDiffuse, FbxSurfaceMaterial::sDiffuseFactor);
	printf("ディフュ?ズカラ?:r = %f, g = %f, b = %f\n", lDiffuse.mData[0], lDiffuse.mData[1], lDiffuse.mData[2]);

	const FbxDouble3 lSpecular = GetMaterialProperty(mat0, FbxSurfaceMaterial::sSpecular, FbxSurfaceMaterial::sSpecularFactor);
	printf("スペキュラカラ?:r = %f, g = %f, b = %f\n", lSpecular.mData[0], lSpecular.mData[1], lSpecular.mData[2]);

	FbxProperty lTransparencyFactorProperty = mat0->FindProperty(FbxSurfaceMaterial::sTransparencyFactor);
	...
	FbxProperty lShininessProperty = mat0->FindProperty(FbxSurfaceMaterial::sShininess);
