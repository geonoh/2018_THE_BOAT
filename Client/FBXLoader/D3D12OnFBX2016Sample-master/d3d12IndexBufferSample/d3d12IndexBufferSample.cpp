...
// 定?
//==============================================================================
#define RESOURCE_SETUP	// リ??スセットアップをする
...
struct UserVertex
{
	DirectX::XMFLOAT3 pos;		// 頂?座標
	DirectX::XMFLOAT4 color;	// 頂?カラ?
...
static const LPTSTR	CLASS_NAME = TEXT("d3d12IndexBufferSample");	// ウィンドウネ??
static const UINT BACKBUFFER_COUNT = 2;					// バックバッフ?数
static const UINT CONSTANT_BUFFER_COUNT = 2;			// コンス?ントバッフ?数
...
enum DESCRIPTOR_HEAP_TYPE
{
	...
	DESCRIPTOR_HEAP_TYPE_RTV,			// レン????ゲット用	
	DESCRIPTOR_HEAP_TYPE_DSV,			// デプスステンシル用	
	DESCRIPTOR_HEAP_TYPE_MAX,			// ディスクリプ?ヒ?プ数
...
ID3D12CommandAllocator*		g_pCommandAllocator;								// コ?ンドアロケ??
ID3D12CommandQueue*			g_pCommandQueue;									// コ?ンドキュ?
IDXGIDevice2*				g_pGIDevice;										// GIデバイス
IDXGIAdapter*				g_pGIAdapter;										// GIア?プ??
IDXGIFactory2*				g_pGIFactory;										// GIフ?クトリ?
IDXGISwapChain3*			g_pGISwapChain;										// GIスワップ?ェ?ン
ID3D12PipelineState*		g_pPipelineState;									// パイプラインステ?ト
ID3D12DescriptorHeap*		g_pDescripterHeapArray[DESCRIPTOR_HEAP_TYPE_MAX];	// ディスクリプ?ヒ?プ配列
ID3D12GraphicsCommandList*	g_pGraphicsCommandList;								// ?画コ?ンドリスト
ID3D12RootSignature*		g_pRootSignature;									// ル?トシグニ?ャ
D3D12_VIEWPORT				g_viewPort;											// ビュ???ト
ID3D12Fence*				g_pFence;											// フェンスオブジェクト
HANDLE						g_hFenceEvent;										// フェンスイベントハンドル
UINT64						g_CurrentFenceIndex = 0;							// 現在のフェンスインデックス

UINT						g_CurrentBuckBufferIndex = 0;						// 現在のバックバッフ?
ID3D12Resource*				g_pBackBufferResource[BACKBUFFER_COUNT];			// バックバッフ?のリ??ス
D3D12_CPU_DESCRIPTOR_HANDLE	g_hBackBuffer[BACKBUFFER_COUNT];					// バックバッフ?ハンドル

ID3D12Resource*				g_pDepthStencilResource;							// デプスステンシルのリ??ス
D3D12_CPU_DESCRIPTOR_HANDLE	g_hDepthStencil;									// デプスステンシルハンドル

LPD3DBLOB					g_pVSBlob;											// 頂?シェ??ブロブ
LPD3DBLOB					g_pPSBlob;											// ピクセルシェ??ブロブ

ID3D12Resource*				g_pVertexBufferResource;							// 頂?バッフ?のリ??ス
D3D12_VERTEX_BUFFER_VIEW	g_VertexBufferView;									// 頂?バッフ?ビュ?

ID3D12Resource*				g_pIndexBufferResource;								// インデックスバッフ?のリ??ス
D3D12_INDEX_BUFFER_VIEW		g_IndexBufferView;									// インデックスバッフ?ビュ?

ID3D12Resource*				g_pConstantBufferResource;							// コンス?ントバッフ?リ??ス
cbMatrix					g_ConstantBufferData;								// コンス?ントバッフ?の実デ??
D3D12_CPU_DESCRIPTOR_HANDLE	g_hConstantBuffer[CONSTANT_BUFFER_COUNT];			// コンス?ントバッフ?ハンドル
...
bool initDirectX(HWND hWnd)
{
	...
	if(showErrorMessage(hr, TEXT("デバッグレイヤ?作成失敗")))
	{
		return false;
	}
	if(g_pDebug)
	...
	if(showErrorMessage(hr, TEXT("GIフ?クトリ獲得失敗")))
	{
		return false;
	}
	...
	if(showErrorMessage(hr, TEXT("GIア?プ??獲得失敗")))
	{
		return false;
	}
	...
	if(showErrorMessage(hr, TEXT("コ?ンドアロケ??作成失敗")))
	{
	...
	descSwapChain.BufferCount = BACKBUFFER_COUNT;					// バックバッフ?は2枚以上ないと失敗する
	...
	if(showErrorMessage(hr, TEXT("スワップ?ェ?ン作成失敗")))
	{
		return false;
	}
	...
	if(showErrorMessage(hr, TEXT("コ?ンドラインリスト作成失敗")))
	{
		return false;
	}
	...
	{
		showErrorMessage(E_FAIL, TEXT("頂?シェ??コンパイル失敗"));
	}

	// ピクセルシェ??コンパイル
	if(compileShaderFlomFile(L"../shader/PixelShader.hlsl", "main", "ps_5_1", &g_pPSBlob) == false)
	{
		showErrorMessage(E_FAIL, TEXT("ピクセルシェ??コンパイル失敗"));
	}

	// 頂?シェ??にコンス?ントバッフ?を渡せるル?トシグニ?ャ作成
	...
	if(showErrorMessage(hr, TEXT("ル?トシグニ?ャ作成失敗")))
	{
		return false;
	}

	// ライス?ライザ?ステ?ト設定
	// デフォルトの設定は以下のペ?ジを参照(ScissorEnableはないけど・・・)
	...
	// パイプラインステ?トオブジェクト作成
	// 頂?シェ??とピクセルシェ??がないと、作成に失敗する
	D3D12_GRAPHICS_PIPELINE_STATE_DESC descPSO;
	ZeroMemory(&descPSO, sizeof(descPSO));
	descPSO.InputLayout = { INPUT_LAYOUT, ARRAYSIZE(INPUT_LAYOUT) };										// インプットレイアウト設定
	descPSO.pRootSignature = g_pRootSignature;																// ル?トシグニ?ャ設定
	descPSO.VS = { reinterpret_cast<BYTE*>(g_pVSBlob->GetBufferPointer()), g_pVSBlob->GetBufferSize() };	// 頂?シェ??設定
	descPSO.PS = { reinterpret_cast<BYTE*>(g_pPSBlob->GetBufferPointer()), g_pPSBlob->GetBufferSize() };	// ピクセルシェ??設定
	descPSO.RasterizerState = descRasterizer;																// ラス?ライザ設定
	descPSO.BlendState = descBlend;																			// ブレンド設定
	descPSO.DepthStencilState = CD3DX12_DEPTH_STENCIL_DESC(D3D12_DEFAULT);									// ステンシルバッフ?有効設定
	descPSO.SampleMask = UINT_MAX;																			// サンプル?スク設定
	descPSO.PrimitiveTopologyType = D3D12_PRIMITIVE_TOPOLOGY_TYPE_TRIANGLE;									// プリ?ティブ?イプ	
	descPSO.NumRenderTargets = 1;																			// レン????ゲット数
	descPSO.RTVFormats[0] = DXGI_FORMAT_R8G8B8A8_UNORM;														// レン????ゲットフォ??ット
	descPSO.SampleDesc.Count = 1;																			// サンプルカウント

	hr = g_pDevice->CreateGraphicsPipelineState(&descPSO, IID_PPV_ARGS(&g_pPipelineState));

	if(showErrorMessage(hr, TEXT("パイプラインステ?トオブジェクト作成失敗")))
	{
		return false;
	}
	...
	for(int i = 0; i < DESCRIPTOR_HEAP_TYPE_MAX; ++i)
	{
		...
		if(showErrorMessage(hr, TEXT("ディスクリプ?ヒ?プ作成失敗")))
		{
			return false;
		}
	}

	// ディスクリプ?ヒ?プとコ?ンドリストの関連づけ
	...
	for(UINT i = 0; i < BACKBUFFER_COUNT; ++i)
	{
		...
		if(showErrorMessage(hr, TEXT("レン????ゲットビュ?作成失敗")))
		{
			return false;
		}
		g_hBackBuffer[i] = g_pDescripterHeapArray[DESCRIPTOR_HEAP_TYPE_RTV]->GetCPUDescriptorHandleForHeapStart();
		g_hBackBuffer[i].ptr += i * strideHandleBytes;	// レン????ゲットのオフセット分?イン?をずらす
	...
	// デプスステンシル作成
	{
		...
		if(showErrorMessage(hr, TEXT("デプスステンシルバッフ?作成失敗")))
		{
			return false;
		}
	...
	g_viewPort.MinDepth = 0.0f;			// 最少?度
	g_viewPort.MaxDepth = 1.0f;			// 最大?度
...
bool setupResource()
{
	...
	// 頂?バッフ?作成
	{
		...
		if(showErrorMessage(hr, TEXT("頂?バッフ?作成失敗")))
		{
			return false;
		}
		...
		{
			showErrorMessage(E_FAIL, TEXT("頂?バッフ?の?ッピングに失敗"));
			return false;
		}
	...
	// インデックスバッフ?作成
	{
		...
		if(showErrorMessage(hr, TEXT("インデックスバッフ?作成失敗")))
		{
			return false;
		}

		// インデックスバッフ?にインデックス情報をコピ?する
		UINT8* dataBegin;
		...
		{
			showErrorMessage(E_FAIL, TEXT("インデックスバッフ?の?ッピングに失敗"));
			return false;
		}
	...
	// コンス?ントバッフ?作成
	{
		...
		if(showErrorMessage(hr, TEXT("コンス?ントバッフ?作成失敗")))
		{
			return false;
		}
		...
		descConstantBufferView.SizeInBytes = (sizeof(cbMatrix) + 255) & ~255;	// コンス?ントバッフ?は256バイトアラインメントで配置する必要がある
...
void Render()
{
	...
	// 1つ目
	{
		...
		{
			showErrorMessage(S_FALSE, TEXT("コンス?ントバッフ?の?ップに失敗しました"));
		}
...
int APIENTRY _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nCmdShow)
{
	...
	winc.lpfnWndProc = WndProc;					// ウィンドウプロシ?ジャ
	winc.cbClsExtra = 0;
	winc.cbWndExtra = 0;
